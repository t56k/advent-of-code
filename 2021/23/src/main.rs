...
pub fn parse(input: &str) -> Vec<Amphipod> {
...
pub fn part_one(input: &Vec<Amphipod>) -> usize {
...
pub fn part_two(input: &Vec<Amphipod>) -> usize {
    let initial_state = State {
        hallway: [None; 11],
        rooms: [
...
fn main() {
    ...
    let amphipods = parse(&input);

    benchmark(|| println!("part one: {}", part_one(&amphipods)));
    benchmark(|| println!("part two: {}", part_two(&amphipods)));
}
